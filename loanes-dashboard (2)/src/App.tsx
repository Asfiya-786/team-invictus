...
import InboxTab, { INITIAL_FEEDBACK_MESSAGES } from './components/InboxTab';
import CustomersTab, { INITIAL_CUSTOMERS } from './components/CustomersTab';
import FraudTab from './components/FraudTab';
import LoansTab from './components/LoansTab';
import EmployeesTab, { INITIAL_EMPLOYEES } from './components/EmployeesTab';
...
import { Transaction, Customer, ActivityLog, CashflowItem, Employee, FeedbackMessage } from './types';
...
export default function App() {
  ...
  const [cashflow, setCashflow] = useState<CashflowItem[]>(CASHFLOW_DATA);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-display font-extrabold text-xl lg:text-2xl text-purple-950 capitalize leading-tight">
                    {currentTab === 'deposits' ? 'Deposits' : currentTab === 'fraud' ? 'Fraud Detection' : currentTab}
                  </h2>
                ...
                return (
                  ...
                  >
                    <DepositsTab 
                      ...
                      customers={customers}
                      onSetCustomers={setCustomers}
                      onAddCashflowIncome={(amount) => {
                        setCashflow(prev => {
                          const next = [...prev];
                          next[5] = {
                            ...next[5],
                            income: next[5].income + amount
                          };
                          return next;
                        });
                      }}
                ...
                return (
                  ...
                  >
                    <EmployeesTab 
                      ...
                      employees={employees}
                      onSetEmployees={setEmployees}
                      onAddInboxMessage={(newMsg) => {
                        setMessages(prev => [newMsg, ...prev]);
                        showToast(`🔔 Notification: Leave request from ${newMsg.sender} received in your Inbox!`);
                      }}
                      onAddActivityLog={(actionMsg) => {
                        const newLog: ActivityLog = {
                          id: `act-new-${Date.now()}`,
