...
interface InboxTabProps {
  ...
  onSetMessages?: React.Dispatch<React.SetStateAction<FeedbackMessage[]>>;
  employees?: Employee[];
...
export default function InboxTab({ 
  ...
  const [internalMessages, setInternalMessages] = useState<FeedbackMessage[]>(INITIAL_FEEDBACK_MESSAGES);
  const messages = propMessages || internalMessages;
  const setMessages = onSetMessages || setInternalMessages;
  ...
  const handleResolveLeave = (approved: boolean) => {
    if (!activeMsg || activeMsg.type !== 'Leave') return;

    const statusLabel = approved ? 'APPROVED' : 'REJECTED';
    setMessages(prev => prev.map(m => {
      if (m.id === activeMsg.id) {
        return {
          ...m,
          subject: `[${statusLabel}] ${m.subject.replace(/\[(APPROVED|REJECTED)\]\s*/g, '')}`,
          isRead: true,
          suggestedAction: `This leave request has been officially ${statusLabel.toLowerCase()} by administration.`
        };
      }
      return m;
    }));

    if (employees && onSetEmployees) {
      onSetEmployees(prev => prev.map(emp => {
        if (emp.name === activeMsg.sender) {
          return {
            ...emp,
            status: approved ? 'On Leave' : 'Active'
          };
        }
        return emp;
      }));
    ...
    if (onAddActivityLog) {
      onAddActivityLog(`Admin ${approved ? 'approved' : 'rejected'} leave request for ${activeMsg.sender}`);
    }

    const timestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const autoIntercomMsg = {
      sender: 'Super Admin' as const,
      text: `SYSTEM ACTIVE NOTICE: Administrative action processed. Leave request for ${activeMsg.sender} has been ${statusLabel}. Employee database status transited to '${approved ? 'On Leave' : 'Active'}'.`,
      date: timestamp
    };
    setChats(prev => ({
      ...prev,
      [activeMsg.id]: [...(prev[activeMsg.id] || []), autoIntercomMsg]
    }));
  };
            ...
            {activeMsg && (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2.5 pt-1 border-t border-purple-950/5">
                  ...
                  {activeMsg.type === 'Leave' ? (
                    <>
                      <button
                        onClick={() => handleResolveLeave(true)}
                        disabled={activeMsg.subject.includes('APPROVED') || activeMsg.subject.includes('REJECTED')}
                        className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold text-[10px] uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow-md shadow-emerald-950/10"
                      >
                      ...
                      <button
                        onClick={() => handleResolveLeave(false)}
                        disabled={activeMsg.subject.includes('APPROVED') || activeMsg.subject.includes('REJECTED')}
                        className="px-4 py-2.5 rounded-xl bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white font-bold text-[10px] uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow-md shadow-rose-950/10"
                      >
