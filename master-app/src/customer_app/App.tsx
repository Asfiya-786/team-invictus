...
export default function App() {
  ...
  // LocalStorage session check & profile fetch from customers table
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      if (!storedUser) {
        console.log('[Customer App] No bank_user found in localStorage - redirecting to landing');
      ...
      // Populate real data from Supabase students table using exact schema column names
      setProfile(prev => ({
        ...prev,
        fullName: customerData.full_name || userData.full_name || prev.fullName,
        customerId: customerData.customer_id || prev.customerId,
        email: customerData.email_address || userData.email_address || prev.email,
        mobile: customerData.mobile_number || userData.mobile_number || prev.mobile,
        aadhaarNumber: prev.aadhaarNumber, // not in students schema
        panNumber: customerData.pan_number || prev.panNumber,
        kycStatus: customerData.kyc_status || prev.kycStatus,
        accountNumber: customerData.account_number || prev.accountNumber,
        accountType: customerData.account_type || prev.accountType,
        branchName: customerData.branch_name || prev.branchName,
        ifscCode: customerData.ifsc_code || prev.ifscCode,
        openingDate: prev.openingDate, // not in students schema
        address: prev.address, // not in students schema
        city: customerData.city || prev.city,
        state: customerData.state || prev.state,
        pinCode: prev.pinCode, // not in students schema
        dob: customerData.date_of_birth || prev.dob,
        gender: customerData.gender || prev.gender,
        nationality: prev.nationality, // not in students schema
        photoUrl: prev.photoUrl, // not in students schema
      }));

      addSecurityLog(`Session established for: ${customerData.full_name || userData.full_name || 'Customer'}`);
      console.log('[Customer App] Profile updated with Supabase data');
    };
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-zinc-900/40 p-6 rounded-3xl border border-white/5 space-y-6 glow-card-purple">
                      ...
                      <div className="text-center space-y-4">
                       ...
                       <div className="absolute inset-0 rounded-full border-12 border-[#5a0a2c]" />

{/* Bright Pink Rotating Ring */}
<div
  className="absolute inset-0 rounded-full border-12 border-dashed border-[#ff4d9d]/50 animate-spin"
