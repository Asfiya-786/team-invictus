...
export default function App() {
  ...
  const [vaults, setVaults] = useState<SavingVault[]>(INITIAL_SAVINGS_VAULTS);
  ...
  // Vault deposits / target adjustments
  ...
  return (
    <div className="min-h-screen bg-[#fbf5f7] text-slate-100 flex relative antialiased selection:bg-[#ec4899]/30 selection:text-[#2e1065]">
      
      {/* Background neon light ambient clouds and pink fluffy clouds background image */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0 bg-gradient-to-tr from-[#fff3f5] via-[#ffebf0] to-[#fff5f7]">
        
        {/* Cloud Layer 1: Base drifting background cloud cover (pastel-pink translucent layer) */}
        <img 
          src={pinkCloudsBg} 
          alt="Pink Clouds Background Base" 
          className="absolute inset-0 w-full h-full object-cover opacity-50 mix-blend-overlay filter brightness-[1.1] contrast-[1.02]" 
          style={{ animation: 'screen-drift 45s infinite ease-in-out' }}
          referrerPolicy="no-referrer"
        />

        {/* Cloud Layer 2: Floating cloud mask A - moves around the screen */}
        <img 
          src={pinkCloudsBg} 
          alt="Pastel Pink Cloud A" 
          className="absolute w-[650px] h-[450px] object-cover rounded-full filter blur-[15px] opacity-25 mix-blend-multiply top-[5%] left-[-8%]"
          style={{ animation: 'wander-1 32s infinite ease-in-out' }}
          referrerPolicy="no-referrer"
        />

        {/* Cloud Layer 3: Floating cloud mask B - moves on high diagonal path */}
        <img 
          src={pinkCloudsBg} 
          alt="Pastel Pink Cloud B" 
          className="absolute w-[750px] h-[500px] object-cover rounded-full filter blur-[20px] opacity-20 mix-blend-multiply bottom-[10%] right-[-10%]"
          style={{ animation: 'wander-2 38s infinite ease-in-out' }}
          referrerPolicy="no-referrer"
        />

        {/* Cloud Layer 4: Ambient glow pastel nodes for perfect colorful accents */}
        <div className="absolute top-[15%] left-[25%] w-[450px] h-[450px] rounded-full bg-gradient-to-tr from-pink-400/20 via-[#ffccd5]/10 to-transparent blur-[75px]" style={{ animation: 'wander-3 24s infinite ease-in-out' }} />
        <div className="absolute bottom-[25%] right-[20%] w-[550px] h-[550px] rounded-full bg-gradient-to-br from-pink-300/15 via-[#ffe5eb]/10 to-transparent blur-[85px]" style={{ animation: 'wander-1 29s infinite ease-in-out' }} />

        {/* Soft custom pastel pink color-blend nodes to ensure warm, luxury aesthetic */}
        <div className="absolute inset-0 bg-[#ffdce3]/15 mix-blend-overlay" />
        <div className="absolute inset-0 bg-[#fff5f7]/25 mix-blend-color" />
      </div>
      ...
      <div className="flex-1 flex flex-col lg:pl-72 z-10 w-full min-h-screen">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      triggerToast('✓ Successfully signed out! (Sandbox Reset State)');
                      addSecurityLog('Customer session signed out.');
                    }}
                      ...
                      {(() => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {debitCard?.isFrozen && (
                              <div className="absolute inset-0 bg-zinc-950/40 backdrop-blur-xs flex items-center justify-center z-20 rounded-3xl">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {categoriesActivityData.map((cat, idx) => (
                          <div 
                            key={idx} 
                            className="bg-zinc-900/60 hover:bg-zinc-900 border border-white/5 hover:border-white/10 rounded-2xl p-4 flex items-center gap-3.5 transition-all cursor-pointer group"
                          >
                            ...
                            <div>
                              <p className="text-[10px] text-slate-500 font-mono uppercase tracking-widest">{cat.title}</p>
                              <p className="text-sm font-bold font-mono text-slate-200 mt-0.5">{cat.total}</p>
                    ...
                    <div className="bg-zinc-900/40 p-6 rounded-3xl border border-white/5 space-y-6 glow-card-purple">
                      ...
                      <div className="text-center space-y-4">
                        ...
                        <div className="relative w-40 h-40 mx-auto flex items-center justify-center">
                          ...
                          <div className="absolute inset-0 rounded-full border-12 border-zinc-950" />
                          <div className="absolute inset-0 rounded-full border-12 border-dashed border-[#ff5e9c]/30 animate-spin" style={{ animationDuration: '40s' }} />
                          <div className="absolute inset-1 rounded-full border-4 border-dashed border-[#b03bfc]/20" />
        ...
        <footer className="bg-zinc-950/60 border-t border-white/5 py-6 text-center text-xs text-slate-500 relative z-10">
          <p className="font-mono tracking-wider font-extrabold text-slate-400 text-gradient-neon uppercase text-[10px]">
            APEX DIGITAL BANKING CONSOLE © 2026
          </p>
          <p className="mt-1 text-[9px] text-slate-600 leading-normal max-w-lg mx-auto">
